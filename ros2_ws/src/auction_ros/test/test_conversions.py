...
def test_heartbeat_round_trip():
    hb = Heartbeat(sender=3, stamp=12.5, idle=True, position=(1.0, 2.5))
...
def test_lease_and_completed_round_trip():
    l = LeaseRenewal(task_id=4, holder=2, stamp=3.25, follower=3)
...
def test_recruit_round_trip():
    r = RecruitAnnouncement(
        leader=1, task_id=4, seq=2, participants=(0, 2),
        epsilon=0.05, quiescence_rounds=8,
    )
    assert conv.recruit_to_core(conv.recruit_to_msg(r)) == r
